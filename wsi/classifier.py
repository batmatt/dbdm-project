...
def random_forest_classification(
    train_set: pd.DataFrame, test_set: pd.DataFrame, trees_number: int, max_depth: int
) -> pd.DataFrame:
    trees = [build_decision_tree(train_set, max_depth) for _ in range(trees_number)]

    results_data = []
    for _, row in test_set.iterrows():
        prediction = make_forest_prediction(row, trees)
        results_data.append({"prediction": prediction, "actual value": row[CLASS]})
